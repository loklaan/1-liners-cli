...
function fuzzyFilterList (text, list, pluck = x => x) {
  ...
  return fuzzy.filter(
    text,
    list,
    { extract: pluck, pre: Tokens.BEFORE, post: Tokens.AFTER }
...
function highlightedTextToElements (text) {
  ...
  return text.split('')
    ...
    .map((part, i) => part.highlighted
      ? React.createElement(Color, {key: `${i}__${part.text}`, bold: true}, [part.text])
      : part.text
    )
}
...
function getDesc () {
  ...
  return `Find and copy the source of a ${link('1-liners', oneLinerUrl)} function.`;
}
...
async function fetchOneLinerCode (fnName) {
  ...
  const prettiered = prettier.format(
    code,
    { semi: false, parser: "babel", printWidth: 40 }
  )
